...
async function decryptVault(vault, password) {
  if (vault.format !== FORMAT) throw new Error(t('crypto.invalidVault'));
  const now = new Date();
  if (vault.expiresAt && new Date(vault.expiresAt) <= now) throw new Error(t('crypto.expired'));
  const maxOpens = Number(vault.maxOpens || 0);
  const openCount = Number(vault.openCount || 0);
  if (maxOpens > 0 && openCount >= maxOpens) throw new Error(t('crypto.maxOpensReached'));
